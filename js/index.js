document.getElementById("tg").addEventListener("click", function () {
  const url = "https://t.me/BioSyntheticLab"; // Укажите нужную ссылку
  window.location.href = url; // Переход по указанной ссылке
});

document.getElementById("x").addEventListener("click", function () {
  const url = "https://x.com/BioSyntheticLab"; // Укажите нужную ссылку
  window.location.href = url; // Переход по указанной ссылке
});
...
async function typeWriterSimultaneously(speed = 50) {
  ...
  const text3 = pre3.textContent.trim();
  const text4 = pre4.textContent.trim();
  const text5 = pre5.textContent.trim();
  const text6 = pre6.textContent.trim();
  ...
  await Promise.all([
    ...
    typeText(pre3, text3, speed),
    typeText(pre4, text4, speed),
    typeText(pre5, text5, speed),
    typeText(pre6, text6, speed),
  ]);
}
...
var HsvColor = function (h, s, v) {
  ...
  r = Math.min(0, Math.round(r * 0));
  g = Math.min(255, Math.round(g * 256));
  b = Math.min(0, Math.round(b * 0));
...
document.querySelectorAll(".tab-button").forEach((button) => {
  button.addEventListener("click", () => {
    // Воспроизводим звук при переключении вкладки
    const audio = document.getElementById("tab-switch-sound");
    audio.play();
...
let totalAnts = 531; // Начальное количество муравьев
let maxRows = 10; // Максимальное количество строк

// Генерация случайного времени (в секундах)
function getRandomTime() {
  return Math.floor(Math.random() * 10) + "s ago";
}

// Создание списка "муравьев"
function addAntRow() {
  const antList = document.getElementById("ant-list");

  // Создаем строку для нового ID
  const row = document.createElement("tr");
  const idCell = document.createElement("td");
  const timeCell = document.createElement("td");

  // Добавляем ID (начиная с 531 и увеличиваем)
  idCell.textContent = totalAnts;
  totalAnts++; // Увеличиваем количество муравьев

  timeCell.textContent = getRandomTime();
  timeCell.className = "joined-time";

  row.appendChild(idCell);
  row.appendChild(timeCell);

  // Добавляем строку в начало списка
  antList.prepend(row);

  // Обновляем отображаемое количество муравьев
  document.getElementById("total-ants").textContent = totalAnts - 1;

  // Если строк больше максимума, удаляем последнюю
  if (antList.children.length > maxRows) {
    antList.removeChild(antList.lastElementChild);
  }
}

// Инициализация — показываем только 530 строку
document.addEventListener("DOMContentLoaded", () => {
  ...
  idCell.textContent = 530; // Показываем только строку с ID 530
  timeCell.textContent = getRandomTime();
  timeCell.className = "joined-time";
  ...
  antList.appendChild(row);

  // Начинаем добавлять новые строки каждые 5 секунд
  setInterval(addAntRow, 5000); // Добавление строк каждые 5 секунд
});

// Запускаем обновление каждые 2000 миллисекунд (2 секунды)
setInterval(updateAnts, 2000);

// Set the initial time (2 minutes 37 seconds)


const clickSound = new Audio('path/to/sound.mp3'); // Укажите путь к вашему звуковому файлу

// Функция для воспроизведения звука
function playClickSound(event) {
  clickSound.currentTime = 0; // Сбрасываем звук, чтобы он начинался заново
  clickSound.play();
}

// Привязываем обработчик ко всем элементам
